...
const generateTribalCenter = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  const marbleMat = getMaterial('smooth', '#f8fafc');
  const goldMat = getMaterial('metal', '#f59e0b', true);
  const floorMat = getMaterial('brick', '#e2e8f0');
  ...
  const globeMat = getMaterial('tech', '#6366f1', true);
...
const generateGovernance = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  const roofMat = getMaterial('concrete', '#475569');
...
const generatePeople = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  const wallMat = getMaterial('brick', '#fca5a5');
  const roofMat = getMaterial('wood', '#9f1239');
  ...
  const leaves = new THREE.Mesh(leavesGeo, getMaterial('concrete', '#16a34a'));
...
const generateProduct = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  addBox(group, 0.1, 1.0, 0.1, towerH, getMaterial('metal', '#cbd5e1'), 0.5, 0.5);
...
const generateMarket = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  addBox(group, baseW, 0.2, baseW, 0, getMaterial('brick', '#fbbf24'));
  ...
  const roofMat = getMaterial('fabric', '#f59e0b');
  const accentMat = getMaterial('fabric', squadColor);
...
const generateResources = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  addBox(group, baseW * 0.6, hangarH, baseW * 0.9, 0, getMaterial('metal', '#475569'), -0.4, 0);
  ...
  addBox(group, 0.2, hangarH + 1, 0.2, 0, getMaterial('metal', '#facc15'), 1.2, -1);
};
...
const generateResidential = (group: THREE.Group, level: number, sizeInTiles: number, squadColor: string) => {
  ...
  addBox(group, w + 0.2, 0.2, w + 0.2, 1, getMaterial('brick', '#9f1239'));
  const roof = new THREE.Mesh(new THREE.ConeGeometry(1, 1, 4), getMaterial('brick', '#9f1239'));
  roof.position.y = 1.5;
  roof.rotation.y = Math.PI / 4;
  group.add(roof);

  addBox(group, 0.05, 2, 0.05, 0, getMaterial('wood', '#5c4033'), 0.5, 0.5);
