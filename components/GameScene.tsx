...
const createTree = (x: number, z: number, scale = 1): THREE.Group => {
  ...
  const leafColor = seed > 0.8 ? 0xf472b6 : seed > 0.3 ? 0x22c55e : 0x15803d;
...
const GameScene: React.FC<GameSceneProps> = ({ gameState, zoomLevel, onTileClick, onBuildingClick }) => {
  ...
  useEffect(() => {
    ...
    const planeMaterial = new THREE.MeshStandardMaterial({ color: 0x86efac, roughness: 1, metalness: 0 });
    ...
    for (let i = 0; i < particlesCount * 3; i++) {
      ...
      if (i % 3 === 1) posArray[i] = Math.random() * 15 + 2;
    }
    ...
    const selectionGeo = new THREE.BoxGeometry(TILE_SIZE, TILE_SIZE, TILE_SIZE);
    const selectionMat = new THREE.LineBasicMaterial({ color: 0x38bdf8 });
    const selectionEdges = new THREE.EdgesGeometry(selectionGeo);
    const selectionOutline = new THREE.LineSegments(selectionEdges, selectionMat);
    ...
    const animate = () => {
      ...
      buildingsGroupRef.current?.children.forEach((building) => {
        ...
        // Per-building type animation (subtle)
        switch (building.userData.type) {
          case BuildingType.MARKET:
            building.rotation.y = Math.sin(timeRef.current * 0.6) * 0.02;
            break;
          case BuildingType.PRODUCT:
            building.rotation.y += 0.003;
            break;
          case BuildingType.GOVERNANCE:
            building.position.y += Math.sin(timeRef.current * 1.5) * 0.05;
            break;
          case BuildingType.RESOURCES:
            building.rotation.z = Math.sin(timeRef.current * 0.8) * 0.01;
            break;
          default:
            break;
        }
      ...
      // Selection outline
      if (selectionOutlineRef.current && buildingsGroupRef.current) {
        const selected = buildingsGroupRef.current.children.find((b) => b.userData.id === gameState.selectedBuildingId);
        if (selected) {
          ...
          selectionOutlineRef.current.position.copy(center);
          selectionOutlineRef.current.scale.set(size.x, size.y, size.z);
          (selectionOutlineRef.current.material as THREE.LineBasicMaterial).color.setHex(0x38bdf8);
        } else {
